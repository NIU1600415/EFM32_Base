...
bool APDS9960_init()
{
	...
	if (!I2C_ReadRegister(APDS9960_ID, &id))
	{
		return false;
	}
...
int APDS9960_readGesture()
{
	...
	while (1)
	{
		...
		if ((gstatus & APDS9960_GVALID) == APDS9960_GVALID)
		{
			...
			if (fifo_level > 0)
			{
				bytes_read = I2C_BlockReadRegister(APDS9960_GFIFO_U,
																					 (uint8_t *)fifo_data,
																					 (fifo_level * 4));
				if (bytes_read == -1)
				{
...
bool APDS9960_readAmbientLight(uint16_t *val)
{
	...
	*val = 0;
	...
	if (!I2C_ReadRegister(APDS9960_CDATAL, &val_byte))
	{
		return false;
	}
	*val = val_byte;
	...
	if (!I2C_ReadRegister(APDS9960_CDATAH, &val_byte))
	{
		return false;
	}
...
bool APDS9960_readRedLight(uint16_t *val)
{
	...
	*val = 0;
	...
	/* Read value from clear channel, low byte register */
	if (!I2C_ReadRegister(APDS9960_RDATAL, &val_byte))
	{
		return false;
	}
	*val = val_byte;
	...
	/* Read value from clear channel, high byte register */
	if (!I2C_ReadRegister(APDS9960_RDATAH, &val_byte))
	{
		return false;
	}
	*val = *val + ((uint16_t)val_byte << 8);
...
bool APDS9960_readGreenLight(uint16_t *val)
{
	...
	*val = 0;
	...
	/* Read value from clear channel, low byte register */
	if (!I2C_ReadRegister(APDS9960_GDATAL, &val_byte))
	{
		return false;
	}
	*val = val_byte;
	...
	/* Read value from clear channel, high byte register */
	if (!I2C_ReadRegister(APDS9960_GDATAH, &val_byte))
	{
		return false;
	}
	*val = *val + ((uint16_t)val_byte << 8);
...
bool APDS9960_readBlueLight(uint16_t *val)
{
	...
	*val = 0;
	...
	/* Read value from clear channel, low byte register */
	if (!I2C_ReadRegister(APDS9960_BDATAL, &val_byte))
	{
		return false;
	}
	*val = val_byte;
	...
	/* Read value from clear channel, high byte register */
	if (!I2C_ReadRegister(APDS9960_BDATAH, &val_byte))
	{
		return false;
	}
	*val = *val + ((uint16_t)val_byte << 8);
...
bool APDS9960_readProximity(uint8_t *val)
{
	*val = 0;
	...
	if (!I2C_ReadRegister(APDS9960_PDATA, val))
	{
		return false;
	}
...
uint8_t getProxIntLowThresh()
{
	...
	if (!I2C_ReadRegister(APDS9960_PILT, &val))
	{
		val = 0;
	}
...
{
	if (!I2C_WriteRegister(APDS9960_PILT, threshold))
	{
		return false;
	}
...
uint8_t getProxIntHighThresh()
{
	...
	if (!I2C_ReadRegister(APDS9960_PIHT, &val))
	{
		val = 0;
	}
...
{
	if (!I2C_WriteRegister(APDS9960_PIHT, threshold))
	{
		return false;
	}
...
uint8_t APDS9960_getLEDDrive()
{
	...
	if (!I2C_ReadRegister(APDS9960_CONTROL, &val))
	{
		return ERROR;
	}
...
bool APDS9960_setLEDDrive(uint8_t drive)
{
	...
	if (!I2C_WriteRegister(APDS9960_CONTROL, val))
	{
		return false;
	}
...
uint8_t getLEDBoost()
{
	...
	if (!I2C_ReadRegister(APDS9960_CONFIG2, &val))
	{
		return ERROR;
	}
...
bool setLEDBoost(uint8_t boost)
{
	...
	if (!I2C_WriteRegister(APDS9960_CONFIG2, val))
	{
		return false;
	}
...
uint8_t getProxGainCompEnable()
{
	...
	if (!I2C_ReadRegister(APDS9960_CONFIG3, &val))
	{
		return ERROR;
	}
...
bool setProxGainCompEnable(uint8_t enable)
{
	...
	if (!I2C_WriteRegister(APDS9960_CONFIG3, val))
	{
		return false;
	}
...
uint8_t getGestureEnterThresh()
{
	...
	if (!I2C_ReadRegister(APDS9960_GPENTH, &val))
	{
		val = 0;
	}
...
{
	if (!I2C_WriteRegister(APDS9960_GPENTH, threshold))
	{
		return false;
	}
...
uint8_t getGestureExitThresh()
{
	...
	if (!I2C_ReadRegister(APDS9960_GEXTH, &val))
	{
		val = 0;
	}
...
{
	if (!I2C_WriteRegister(APDS9960_GEXTH, threshold))
	{
		return false;
	}
...
uint8_t APDS9960_getGestureGain()
{
	...
	if (!I2C_ReadRegister(APDS9960_GCONF2, &val))
	{
		return ERROR;
	}
...
bool APDS9960_setGestureGain(uint8_t gain)
{
	...
	if (!I2C_WriteRegister(APDS9960_GCONF2, val))
	{
		return false;
	}
...
bool APDS9960_getLightIntLowThreshold(uint16_t *threshold)
{
	...
	*threshold = 0;
	...
	if (!I2C_ReadRegister(APDS9960_AILTL, &val_byte))
	{
		return false;
	}
	*threshold = val_byte;
	...
	if (!I2C_ReadRegister(APDS9960_AILTH, &val_byte))
	{
		return false;
	}
...
bool APDS9960_setLightIntLowThreshold(uint16_t threshold)
{
	...
	/* Write low byte */
	if (!I2C_WriteRegister(APDS9960_AILTL, val_low))
	{
		return false;
	}

	/* Write high byte */
	if (!I2C_WriteRegister(APDS9960_AILTH, val_high))
	{
		return false;
	}
...
bool APDS9960_getLightIntHighThreshold(uint16_t *threshold)
{
	...
	*threshold = 0;
	...
	if (!I2C_ReadRegister(APDS9960_AIHTL, &val_byte))
	{
		return false;
	}
	*threshold = val_byte;
	...
	if (!I2C_ReadRegister(APDS9960_AIHTH, &val_byte))
	{
		return false;
	}
	*threshold = *threshold + ((uint16_t)val_byte << 8);
...
bool APDS9960_setLightIntHighThreshold(uint16_t threshold)
{
	...
	/* Write low byte */
	if (!I2C_WriteRegister(APDS9960_AIHTL, val_low))
	{
		return false;
	}

	/* Write high byte */
	if (!I2C_WriteRegister(APDS9960_AIHTH, val_high))
	{
		return false;
	}
...
bool APDS9960_getProximityIntLowThreshold(uint8_t *threshold)
{
	*threshold = 0;
	...
	if (!I2C_ReadRegister(APDS9960_PILT, threshold))
	{
		return false;
	}
...
bool APDS9960_getProximityIntHighThreshold(uint8_t *threshold)
{
	*threshold = 0;
	...
	/* Read value from proximity low threshold register */
	if (!I2C_ReadRegister(APDS9960_PIHT, threshold))
	{
		return false;
	}
...
uint8_t APDS9960_getAmbientLightIntEnable()
{
	...
	if (!I2C_ReadRegister(APDS9960_ENABLE, &val))
	{
		return ERROR;
	}
...
bool APDS9960_setAmbientLightIntEnable(uint8_t enable)
{
	...
	if (!I2C_WriteRegister(APDS9960_ENABLE, val))
	{
		return false;
	}
...
{
	uint8_t throwaway;
	if (!I2C_ReadRegister(APDS9960_AICLEAR, &throwaway))
	{
		return false;
	}
...
{
	uint8_t throwaway;
	if (!I2C_ReadRegister(APDS9960_PICLEAR, &throwaway))
	{
		return false;
	}
